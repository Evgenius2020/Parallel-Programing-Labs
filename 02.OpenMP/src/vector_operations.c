...
double scalar_vector_x_vector(double *left, double *right, unsigned vector_size)
{
    ...
    unsigned i;
#ifdef BUILD_OMP_HARD
#pragma omp parallel for shared(left, right, result) private(i)
#endif
    for (i = 0; i < vector_size; i++)
    {
...
void vector_sub_vector(double *left, double *right, unsigned vector_size)
{
    unsigned i;
#ifdef BUILD_OMP_HARD
#pragma omp parallel for shared(left, right) private(i)
#endif
    for (i = 0; i < vector_size; i++)
    {
        left[i] -= right[i];
...
double vector_norm(double *vector, unsigned vector_size)
{
    ...
    unsigned i;
#ifdef BUILD_OMP_HARD
#pragma omp parallel for shared(vector, result) private(i)
#endif
    for (i = 0; i < vector_size; i++)
    {
