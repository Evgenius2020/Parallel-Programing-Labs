...
double calculate_next_matrix(Parameters parameters, Local_Data local_data)
{
    double max_delta = 0;    
    int i, j;
    for (i = 0; i < local_data.matrix_height; i++)
    {
        for (j = 0; j < local_data.matrix_width; j++)
        {
            ...
            double delta = sqrt(pow(phi_next - phi_center, 2));
            if (delta > max_delta)
            {
                max_delta = delta;
            }
            local_data.curr_matrix[i * local_data.matrix_width + j] = phi_next;            
        }
    }
    return max_delta;
}
         ...
         MPI_Datatype column_datatype)
{
    ...
    double max_delta = calculate_next_matrix(parameters, local_data);
